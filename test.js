...
class final_result {
    ...
    date;
    time;
    constructor(name, gender, age, final_score, date, time) {
        this.name = name;
        this.age = age;
        this.gender = gender;
...
async function Quiz() {
    ...
    if (startOrNot.confirm === 'Yes') {
        ...
        if (Questions.Q1 === '(B) parseInt()') {
            score++;
        }
        if (Questions.Q2 === '(B) function myFunction() {}') {
            score++;
        }
        if (Questions.Q3 === '(C) Block scope') {
            score++;
        }
        if (Questions.Q4 === '(D) All of the above') {
            score++;
        }
        if (Questions.Q5 === '(A) push()') {
            score++;
        }
        if (Questions.Q6 === '(B) .catch()') {
            score++;
        }
        if (Questions.Q7 === '(B) Declares a function that returns a Promise') {
            score++;
        }
        if (Questions.Q8 === '(B) doubleClick') {
            score++;
        }
        if (Questions.Q9 === '(B) object') {
            score++;
        }
        if (Questions.Q10 === '(C) getElementById()') {
            score++;
        }
    }
    else {
